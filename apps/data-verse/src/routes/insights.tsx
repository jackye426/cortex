...
import { crossChannels, crossReadouts, sourceLabel } from "@/lib/overlays";
...
function InsightPage() {
  const { data, degraded, loading } = useDensity("cross");
  const channels = useMemo(
    () => (loading ? undefined : crossChannels(data)),
    [data, loading],
  );
  const readouts = useMemo(() => crossReadouts(data), [data]);
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[minmax(0,1fr)_240px]">
        <section className="flex min-h-0 flex-col overflow-hidden">
          <div className="dv-micro flex items-center justify-between border-b border-dv-hair px-3 py-2 text-dv-faint">
            ...
            <span className="hidden sm:inline">
              {readouts.halfLabels.top} ▲ / {readouts.halfLabels.bottom} ▼
            </span>
          </div>
          <div className="min-h-0 flex-1">
            <InsightField
              nodeCount={4600}
              channels={channels}
              halfLabels={readouts.halfLabels}
              {...(loading ? {} : { halfWeights: readouts.halfWeights })}
            />
          </div>
        ...
        <div className="hidden min-h-0 overflow-hidden md:flex md:flex-col md:border-l md:border-dv-line">
          <div className="min-h-0 flex-1 overflow-hidden">
            <IndexColumn
              title="THEMES / FAMILIES"
              {...(loading ? {} : { meters: readouts.themes })}
            />
          </div>
          <NumericMatrix title="MATRIX / DELTA" seed={7788} rows={6} />
        </div>
      </div>
    </DvFrame>
  );
