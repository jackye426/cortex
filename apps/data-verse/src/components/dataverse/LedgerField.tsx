...
function DetailPane({ row }: { row: VizLedgerRow | undefined }) {
  ...
  async function verdict(v: "confirm" | "reject" | "refine") {
    ...
    const res = await postVerdict({
      ...
      verdict: v,
      note: note || undefined,
      claim: v === "refine" ? note || undefined : undefined,
    });
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-auto">
      ...
      <div className="space-y-3 px-3 py-3 font-mono text-[11px] leading-relaxed text-dv-dim">
        ...
        {d.kind === "mirror" && (
          <>
            <Block label="NOTICE" body={d.notice} />
            <Block label="WHY" body={d.why} />
            <Block label="EVIDENCE" body={`N=${d.evidenceCount}  CONF=${d.confidence.toFixed(3)}`} />
            <Block label="RIVAL" body={d.rival} />
            <Block label="TEST" body={d.test} />
            {d.contradictions.length > 0 && (
              <Block label="CONTRA" body={d.contradictions.join(" | ")} />
            )}
            {(d.controls.confirm || d.controls.reject || d.controls.refine) && (
              <div className="space-y-2 border border-dv-hair p-2">
                <div className="dv-micro text-dv-faint">VIR CONTROLS</div>
                <input
                  className="w-full border border-dv-hair bg-transparent px-2 py-1 text-dv-fg outline-none"
                  placeholder="NOTE / REFINE CLAIM"
                  value={note}
                ...
                <div className="flex flex-wrap gap-2">
                  {d.controls.confirm && (
                    <button
                      type="button"
                      className="border border-dv-line px-2 py-1 text-dv-fg hover:bg-dv-fg hover:text-dv-bg"
                      onClick={() => void verdict("confirm")}
                    >
                      CONFIRM
                    </button>
                  )}
